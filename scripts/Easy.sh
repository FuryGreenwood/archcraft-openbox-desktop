...
change_xfce_terminal() {
	...
	sed -i -e 's/ColorForeground=.*/ColorForeground=#FFFFFFFFFFFF/g' 	${xfce_term_path}/terminalrc
	sed -i -e 's/ColorBackground=.*/ColorBackground=#25252A2A3535/g' 	${xfce_term_path}/terminalrc
	sed -i -e 's/ColorCursor=.*/ColorCursor=#FFFFFFFFFFFF/g' 			${xfce_term_path}/terminalrc
...
change_rofi 'easy' 'Iosevka 12' '1px 1px 1px 1px' '0px' 'Arc-Circle'

# funct STYLE (network manager applet)
change_nm 'easy'

# funct FONT SIZE (Change colors in funct)
change_terminal 'JetBrainsMono Nerd Font' '12'

# funct FONT (Change colors in funct)
change_xfce_terminal 'JetBrainsMono Nerd Font 12'

# funct SCHEME FONT
change_geany 'arc' 'JetBrains Mono 12'
